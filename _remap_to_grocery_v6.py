"""Remap Teatower SKU vers schema GROCERY 2026 (v6).

Differences vs v5 :
- Etend SKIP_SKUS pour exclure :
  * 205 SKU traites via offer feed separe (erreur 8541 - ASIN existe deja sur Amazon)
  * 15 SKU non-thes non vendables sur FBA (erreur 8560 - data insuffisante)
    Cheques cadeaux, ateliers physiques, accessoires sans EAN, Z0999
  * 26 SKU bloques par autres erreurs a traiter plus tard (5995, 100873, 100980, 8542, 18027, 99001/99016/99038)
- Conserve les 6 SKU originaux (4 TEA conflit + 2 brand registry only)
- Charge le SKIP etendu depuis data/_v6_skip_sku.json

Total SKIP : 6 + 205 + 15 + 26 = 252 SKU
Reste v6 listing : 351 - 252 = 99 SKU

Output:
  - data/GROCERY_Teatower_v6_ready.xlsx (xlsx editable)
  - data/GROCERY_Teatower_v6_ready.txt  (CP1252 tab-delimited pour Seller Central)
"""
import openpyxl
import unicodedata
import sys, io
...
OUT_XLSX = './data/GROCERY_Teatower_v6_ready.xlsx'
OUT_TXT = './data/GROCERY_Teatower_v6_ready.txt'
SKIP_JSON = './data/_v6_skip_sku.json'
...
# SKIP_SKUS construit a partir du JSON + 6 originaux
SKIP_SKUS = {
    "I0735", "I0723", "I0628", "I0205",  # conflit ASIN TEA (v5)
    "C0184", "I0669",                      # brand registry only (v5)
}
with open(SKIP_JSON, 'r', encoding='utf-8') as _f:
    _skip = json.load(_f)
SKIP_SKUS |= set(_skip['offer'])     # 205 SKU vers offer feed (8541)
SKIP_SKUS |= set(_skip['non_tea'])   # 15 SKU non-thes (8560)
SKIP_SKUS |= set(_skip['later'])     # 26 SKU a traiter plus tard
print(f'SKIP_SKUS total : {len(SKIP_SKUS)} SKU')

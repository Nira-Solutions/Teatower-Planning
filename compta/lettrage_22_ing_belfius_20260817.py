# -*- coding: utf-8 -*-
"""
LETTRAGE ING + BELFIUS 17/08/2026

Demande Nicolas : "lettre tout ce que tu trouves dans ING et belfius, tu sais quoi faire
pour les factures de clients, regarde aussi les achats qu'on a fait."

Perimetre scanne : 102 lignes non lettrees sur les journaux 14 (ING BE30 3631 6408 2311)
et 36 (Belfius BE86 0689 5807 1350).

Regle d'ecart (cf feedback_compta_ecart_reglement_tolerance) :
...
Types de traitement :
  ...
  D INTERNE  : virement TEATOWER ING <-> Belfius, les 2 miroirs -> 580000 puis lettres

NON TRAITES VOLONTAIREMENT (voir RECAP en fin d'execution) : voir bloc NON_TRAITES.

NOTE CONFIDENTIALITE : repo PUBLIC, pas de secret ici (ODOO_PWD en variable d'env).

Usage : python lettrage_22_ing_belfius_20260817.py            (DRY-RUN)
        python lettrage_22_ing_belfius_20260817.py --apply
"""
...
P_VILNA = 3286     # Vilna Gaon S.R.L (entite d'encaissement Mollie)

DRY = "--apply" not in sys.argv

# ---------------------------------------------------------------------------
# A -- pieces identifiees (client ET fournisseur)
# ---------------------------------------------------------------------------
CASES_MATCH = [
    # CLIENT : la communication nomme litteralement la facture ET la note de credit.
    # "2026-03529-nc26/27-0012" = INV/2026/03529 (332,50) - RINV/26-27/0012 (26,60) = 305,90
    {"bsl_ids": [20365], "acc": ACC_400000, "docs": ["INV/2026/03529"],
     "label": "Pharmacie Badot - INV/2026/03529 residuel 305,90 (comm 2026-03529-nc26/27-0012, exact)"},

    # ACHAT : la domiciliation porte "/INV/2260310704 9.6.2026" = ref exacte de la facture
    # fournisseur RESA1059 Worldline. Montant preleve 529,78 < facture 626,31 -> lettrage
    # PARTIEL assume (ecart 96,53 > tolerance 5 EUR, JAMAIS de write-off a ce niveau).
    {"bsl_ids": [19809], "acc": ACC_440000, "docs": ["RESA1059"], "allow_partial": True,
     "label": "Worldline - RESA1059 ref 2260310704 (facture 626,31 / preleve 529,78, partiel -96,53)"},
]

# ---------------------------------------------------------------------------
# B -- repointage sans lettrage (comptes de transit etablis)
# ---------------------------------------------------------------------------
CASES_TRANSIT = [
    # Titres-repas -> 580003 (precedents : BSL 20278, 20141, 20163, 20230, 19334...)
    {"bsl": 20363, "acc": ACC_580003, "partner": None,
     "label": "EDENRED BELGIUM 9,83 - titres-repas"},
    {"bsl": 20364, "acc": ACC_580003, "partner": None,
     "label": "EDENRED BELGIUM 29,49 - titres-repas"},
    # Bons cadeaux -> 580004 (precedent etabli)
    {"bsl": 20393, "acc": ACC_580004, "partner": None,
     "label": "SMARTBOX GROUP 81,18 - bons cadeaux (PCI-001943655)"},
    # Versements Mollie -> 400000 / Vilna Gaon (precedents BSL 19189 10/06 et 19285 24/06)
    {"bsl": 19573, "acc": ACC_400000, "partner": P_VILNA,
     "label": "Versement Mollie / VILNA GAON 3.028,81 (08/07)"},
    {"bsl": 19758, "acc": ACC_400000, "partner": P_VILNA,
     "label": "Versement Mollie / VILNA GAON 4.427,07 (16/07)"},
    # Virement interne 08/06 Belfius -> ING : la contrepartie ING (BSL 18955) a DEJA ete
    # repointee sur 580001 avec residual 0,00 -> rien a lettrer, on miroite le compte.
    {"bsl": 19186, "acc": ACC_580001, "partner": None,
     "label": "Virement interne 08/06 Belfius -3.000 -> ING (miroir de BSL 18955 deja traite sur 580001)"},
]
...
# ---------------------------------------------------------------------------
CASES_INTERNAL = [
    {"a": 20375, "b": 20323, "label": "13/08 ING -10.000,00 -> Belfius +10.000,00 (Stock)"},
]

# ---------------------------------------------------------------------------
NON_TRAITES = [
    ("ING  20156", "CARREFOUR BELGIUM      +4.862,06", "comm /ADV/ sans detail, 55+ factures ouvertes, aucun sous-ensemble unique -> exiger l'avis de paiement"),
    ("ING  20381", "DELHAIZE LE LION       +2.652,97", "/ADV/2000058526 : 88 factures ouvertes cote central, 0 solution exacte a <=3 factures -> exiger l'avis"),
    ("ING  19660", "ITM ALIMENTAIRE BELGIUM  +637,24", "0000287398 : aucune combinaison exacte sur les 22 factures Intermarche ouvertes -> exiger l'avis"),
    ("ING  20373", "ITM ALIMENTAIRE BELGIUM  +160,61", "0000290027 : idem, aucune combinaison exacte -> exiger l'avis"),
    ("ING  19625", "KIRCHNER domiciliation -12.837,54", "46 factures ouvertes (161.522,58), 0 solution exacte <=4 factures -> exiger l'Avis du 07.07.26"),
    ("BELF 20352", "KIRCHNER clearing     -49.551,11", "idem, 0 solution exacte -> exiger le detail du clearing"),
    ("ING  19992", "iPiD Europe                +0,01", "micro-depot de verification de compte, pas un reglement"),
    ("ING  19871", "RADIUS                   -308,80", "la facture RESA1201 (ref BE261701699965) est DEJA lettree... contre des lignes bancaires de MARS -> lettrage croise a defaire, hors perimetre"),
    ("ING  19985", "RADIUS                   -459,81", "idem RESA1195 lettree contre BNK1/25-26/4271 et /4394 (mars) -> lettrage croise a defaire"),
    ("ING  19944", "PROXIMUS                 -200,00", "RESA1197 lettree contre des soldes d'ouverture de dec-2025 -> lettrage croise a defaire"),
    ("ING  19780 / 20178", "SENDCLOUD  -24,45 / -121,00", "aucune facture d'achat enregistree pour ces refs (1-26-BE0060716 / BE0064771)"),
    ("ING  divers", "GOOGLE / ADOBE / INTUIT / SHOPIFY / META", "domiciliations sans facture d'achat enregistree en compta"),
    ...
    ("BELF x14", "CHARGES ECHUES CREDIT PRO", "706,91 / 273,64 / 410,46 / 2.559,85 : necessitent le tableau d'amortissement (split capital/interets)"),
    ("BELF x12 + ING x2", "FRAIS BANCAIRES", "tenue de compte, avis, cash plus, interets, cotisation MasterCard -> compte de charge a arbitrer (6570xx vs parking 440000)"),
    ("ING 20157/20288/19735/20087 + avances", "SALAIRES ET AVANCES", "l'OD de paie juin+juillet n'est pas comptabilisee (cf project_salaires_non_comptabilises_juin_juillet_2026) -> rien a lettrer tant qu'elle manque"),
    ...
    ("ING / BELF divers", "PETITS ACHATS CARTE", "Braderie, Le Fleuron, Familia, CRF Express, Get your mug, Google Play : notes de frais a saisir"),
]
...
def create_writeoff(partner_id, amount_abs, debit_acc, credit_acc, label):
    vals = {
        ...
        "date": TODAY,
        "ref": f"Write-off ecart reglement lettrage ING/Belfius 17/08 - {label}",
        "line_ids": [

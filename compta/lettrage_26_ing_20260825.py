# -*- coding: utf-8 -*-
"""
LETTRAGE ING 25/08/2026

Demande Nicolas : "lettre ce que tu vois dans ING".

Perimetre : journal 14 (ING). 95 lignes non lettrees ING+Belfius au 25/08/2026.
Seuls les cas CORROBORES sont traites : la communication structuree BE reste la
cle maitre (reference_lettrage_ing_methode).
...
Usage : python lettrage_26_ing_20260825.py            (DRY-RUN)
        python lettrage_26_ing_20260825.py --apply
"""
...
CASES_MATCH = [
    ...
    # "***000/0042/65572*** 000004265572" -> INV/2026/03524 (payment_reference
    # +++000/0042/65572+++), residuel 777,07 = montant vire au centime. Le payeur
    # (BOUCHERIE MAGEROTTE, IBAN BE14 0689 4847 1683) est bien Teroir de Magerotte #121553.
    {"bsl_ids": [20528], "acc": ACC_400000, "docs": ["INV/2026/03524"],
     "label": "BOUCHERIE MAGEROTTE / Teroir de Magerotte 777,07 - comm 000/0042/65572 (exact)"},

    # --- Fournisseur : domiciliation SEPA, IBAN emetteur + montant exact + une seule
    # facture ouverte a ce nom. RESA1243 = FC26356017 du 04/08, 2.299,22 -> preleve le 20/08.
    {"bsl_ids": [20488], "acc": ACC_440000, "docs": ["RESA1243"],
     "label": "ING Equipment Lease 2.299,22 - domiciliation vs RESA1243 / FC26356017 (exact)"},
]
...
# ---------------------------------------------------------------------------
NON_TRAITES = [
    ("ING  20381", "DELHAIZE LE LION       +2.652,97", "avis /ADV/2000058526 toujours non fourni ; subset-sum sur les 91 factures ouvertes = 18 combinaisons <=4 pieces, aucune unique -> exiger l'avis"),
    ("ING  19660 / 20373", "ITM ALIMENTAIRE  +637,24 / +160,61", "ITM Alimentaire Belgium n'existe pas comme partenaire (IBAN BE75 3701 0623 0851) ; aucune combinaison sur les factures Intermarche ouvertes -> exiger l'avis (refs 0000287398 / 0000290027)"),
    ...
    ("ING  19625", "KIRCHNER FISCHER    -12.837,54", "domiciliation ; 46 factures fournisseur ouvertes, aucun subset unique -> exiger l'avis de prelevement"),
    ("ING  19944", "PROXIMUS                 -200,00", "domiciliation 200,00 : aucune facture Proximus ouverte a ce montant (RESA1239 986,78 / RESA1237 122,98 / RBILL 49,61) -> acompte ou facture non encodee"),
    ("ING  divers", "Radius / Sendcloud / Google Cloud / Skeepers / CILE / Douanes", "aucune facture d'achat ouverte au nom de ces fournisseurs -> rien a lettrer, ce sont des charges directes a imputer"),
    ("BELFIUS", "26 lignes", "hors demande (\"lettre ce que tu vois dans ING\") - inchangees depuis la passe du 24/08"),
]
...
def create_writeoff(partner_id, amount_abs, debit_acc, credit_acc, label):
    vals = {
        ...
        "date": TODAY,
        "ref": f"Write-off ecart reglement lettrage ING 25/08 - {label}",
        "line_ids": [

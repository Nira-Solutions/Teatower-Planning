...
- **Périmètre verrouillé (demande Nicolas)** : un bon n'est traité que si les TROIS conditions tiennent — picking type d'un des 3 magasins, `deliveryMethod.methodType == PICK_UP` côté Shopify, ET `assignedLocation.location.id` égal à la location Shopify du magasin qui a validé (Liège 84371767635, Namur 84371865939, Waterloo 108808798547). Un envoi normal expédié depuis un magasin, ou un retrait à Somme-Leuze, est écarté. Vérifié sur 4 cas réels : #49531 (SHIPPING) écarté, #49518 (retrait Somme-Leuze) écarté, #49524 retenu sur Waterloo mais écarté quand on le teste contre Liège.
- **Script** : `odoo/shopify_retrait_pret.py` (audit par défaut, `--apply`, `--fenetre N` jours). Trois garde-fous supplémentaires : fenêtre 5 j, statut du FO côté Shopify (seul un OPEN est marqué), drapeau `x_shopify_pret` sur stock.picking. Vérifié : sur 40 jours d'historique, 8 bons, 0 e-mail — tous déjà IN_PROGRESS/CLOSED. Deuxième passage = « rien à traiter ».
